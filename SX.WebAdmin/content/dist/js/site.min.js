...
function getGridViewData(i,t,a){$grid=$(i),$pager=$grid.find(".sx-gv__pager");var r=$grid.data("ajax-url"),e={};$inputs=$grid.find(".sx-gv__filter-row input"),$inputs.each(function(){$input=$(this);var i=$input.val();if(""!=i){var t=$input.attr("name");e[t]=i}});var d={filterModel:e,order:a,page:t};$.ajax({url:r,data:d,method:"post",success:function(i){$grid.html(i)},complete:function(){$grid.find('[data-toggle="tooltip"]').tooltip()}})}!function(i){i.fn.sx_gv=function(){window.innerWidth>0?window.innerWidth:screen.width;this.each(function(){var t=i(this).closest("div");t.on("click",".sx-gv__pager a",function(){$a=i(this),$grid=$a.closest(".sx-gv");var t=$a.closest("li").hasClass("active");if(!t){var a=$a.data("page");if($order=$grid.find(".sx-gv__sort-arow"),1==$order.length){var r=$order.closest("th").data("field-name"),e=$order.data("sort-direction"),d={FieldName:r,Direction:e};getGridViewData($grid,a,d)}else getGridViewData($grid,a)}}),t.on("keypress",".sx-gv__filter-row input",function(t){if(13==t.which){$input=i(this),$grid=$input.closest(".sx-gv");var a=$grid.find(".sx-gv__pager li.active a").data("page");getGridViewData($grid,a)}}),t.on("click",".sx-gv__clear-btn",function(){$a=i(this),$grid=$a.closest(".sx-gv"),$grid.find(".sx-gv__filter-row input").val(null),getGridViewData($grid,1)}),t.on("click","th",function(){if(!i(this).hasClass("sx-gv_first-column")){$grid=i(this).closest(".sx-gv");var t=$grid.find(".sx-gv__pager li.active a").data("page");$order=i(this).find(".sx-gv__sort-arow");var a=i(this).data("field-name"),r="Asc";if(1==$order.length){var e=$order.data("sort-direction");"Asc"==e&&(r="Desc")}var d={FieldName:a,Direction:r};getGridViewData($grid,t,d)}})})}}(jQuery);